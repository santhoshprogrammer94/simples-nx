...
export class FormComponent extends FormDlgApiComponent implements OnInit, AfterViewInit, OnDestroy {
  constructor(private injector: Injector, @Inject('env') public env, public dlgRef: DialogRef) {
    super(injector, env, dlgRef);

    this.operation = dlgRef.data.operation;

    this.formCRUD = new FormGroup<Menu, NgValidatorsErrors>(
      {
